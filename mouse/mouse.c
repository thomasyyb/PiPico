...
int main() {
    ...
    // initialize the ps2 pio
    uint ps2_offset = pio_add_program(pio1, &ps2_program);
    uint ps2_sm = 0;
    float clkdiv = 3125.f;  // 40kHz @ 125 clk_sys
    ps2_program_init(pio1, ps2_sm, ps2_offset, clkdiv, PS2_CLK, PS2_DATA);
