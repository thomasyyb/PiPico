	...
	case OV5642:
		{
			...
			{	
				...
				if (m_fmt == JPEG)
				{
					...
					sleep_ms(100);
					wrSensorReg16_8(0x3818, 0xa8);  // Mirror off, vertical flip on, thumbnail disabled, compression enabled
					wrSensorReg16_8(0x3621, 0x10);  // array control
					wrSensorReg16_8(0x3801, 0xb0);  // HREF horizontal start point low byte[7:0]
					wrSensorReg16_8(0x4407, 0x04);  // compression control: quantization scale[5:0] = 6'b000100
				}
				else
				{
					byte reg_val;
